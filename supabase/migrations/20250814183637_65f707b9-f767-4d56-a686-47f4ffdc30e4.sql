-- Drop the existing function and recreate it as a simple function (not trigger)
DROP FUNCTION IF EXISTS refresh_financial_summaries();

-- Create the function to refresh financial summaries
...
-- Create a trigger function to call the refresh function
CREATE OR REPLACE FUNCTION trigger_refresh_financial_summaries()
RETURNS trigger AS $$
BEGIN
  PERFORM refresh_financial_summaries();
...
FOR EACH STATEMENT
EXECUTE FUNCTION trigger_refresh_financial_summaries();
